...
export class MessageService {
  ...
  private apiURL = "http://localhost:8000/apimessage";
  ...
  create(message:Message): Observable <any> {
    ...
    return this.httpClient.post(this.apiURL + '/messages/create', JSON.stringify(message), this.httpOptions)
    .pipe(
      catchError(this.errorHandler)
    )
  }  
  ...
  update(id:number, message:Message): Observable <any> {
    ...
    return this.httpClient.put(this.apiURL + '/messages/' + id, JSON.stringify(message), this.httpOptions)
    .pipe( 
      catchError(this.errorHandler)
    )
  }
